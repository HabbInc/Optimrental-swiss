...
import { Car, Check, ChevronLeft, ChevronRight, X, Fuel, Gauge, Calendar, Users, Snowflake, Baby, Scale, Ruler } from 'lucide-react';
                ...
                <DialogContent className="rounded-3xl max-w-4xl p-0 border-none overflow-hidden sm:rounded-3xl max-h-[90vh] overflow-y-auto">
                    <div className="relative">
                            ...
                            <div className="absolute top-6 right-6">
                                <div className="bg-white/90 backdrop-blur-md px-5 py-3 rounded-full shadow-lg">
                                    <span className="text-slate-900 font-black text-xl">
                                        {vehicle.price_per_hour} <span className="text-xs uppercase text-slate-500 tracking-widest">CHF/day</span>
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 space-y-6">
                            <div className="space-y-2">
                                <DialogTitle className="text-3xl font-black text-slate-900">{vehicle.name}</DialogTitle>
                                {(vehicle.manufacturer || vehicle.model) && (
                                    <p className="text-lg text-amber-600 font-bold">
                                        {vehicle.manufacturer} {vehicle.model} {vehicle.year && `(${vehicle.year})`}
                                    </p>
                                )}
                                <p className="text-slate-500 text-base leading-relaxed">{vehicle.description}</p>
                            </div>

                            {/* Features */}
                            <div className="flex flex-wrap gap-2">
                                {(vehicle.features && vehicle.features.length > 0 ? vehicle.features : ['Premium Audio', 'GPS Navigation', 'AC', 'Automatic']).map((tag) => (
                                    <div key={tag} className="flex items-center gap-1.5 bg-slate-100 px-4 py-2 rounded-full text-xs font-bold text-slate-600 uppercase tracking-widest">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4">
                                {vehicle.seat_count && (
                                    <div className="bg-slate-50 p-4 rounded-2xl">
                                        ...
                                        <p className="text-lg font-bold text-slate-900">{vehicle.seat_count}</p>
                                    </div>
                                )}
                                {vehicle.transmission && (
                                    <div className="bg-slate-50 p-4 rounded-2xl">
                                        <div className="flex items-center gap-2 text-slate-400 mb-1">
                                            <Gauge size={16} />
                                            <span className="text-[10px] font-bold uppercase tracking-widest">Transmission</span>
                                        </div>
                                        <p className="text-lg font-bold text-slate-900">{vehicle.transmission}</p>
                                    </div>
                                )}
                                {vehicle.fuel_type && (
                                    <div className="bg-slate-50 p-4 rounded-2xl">
                                        <div className="flex items-center gap-2 text-slate-400 mb-1">
                                            <Fuel size={16} />
                                            <span className="text-[10px] font-bold uppercase tracking-widest">Fuel Type</span>
                                        </div>
                                        <p className="text-lg font-bold text-slate-900">{vehicle.fuel_type}</p>
                                    </div>
                                )}
                                {vehicle.year && (
                                    <div className="bg-slate-50 p-4 rounded-2xl">
                                        ...
                                        <p className="text-lg font-bold text-slate-900">{vehicle.year}</p>
                                    </div>
                                )}
                                {vehicle.euro_class && (
                                    <div className="bg-slate-50 p-4 rounded-2xl">
                                        <div className="flex items-center gap-2 text-slate-400 mb-1">
                                            <Check size={16} />
                                            <span className="text-[10px] font-bold uppercase tracking-widest">Emission Class</span>
                                        </div>
                                        <p className="text-lg font-bold text-slate-900">{vehicle.euro_class}</p>
                                    </div>
                                )}
                            </div>

                            {/* Dimensions */}
                            {(vehicle.width || vehicle.length || vehicle.height || vehicle.curb_weight || vehicle.max_gross_weight) && (
                                <div className="pt-4">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                        {vehicle.length && (
                                            <div className="bg-slate-50 p-4 rounded-2xl text-center">
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Length</p>
                                                <p className="text-lg font-bold text-slate-900">{vehicle.length} cm</p>
                                            </div>
                                        )}
                                        {vehicle.width && (
                                            <div className="bg-slate-50 p-4 rounded-2xl text-center">
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Width</p>
                                                <p className="text-lg font-bold text-slate-900">{vehicle.width} cm</p>
                                            </div>
                                        )}
                                        {vehicle.height && (
                                            <div className="bg-slate-50 p-4 rounded-2xl text-center">
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Height</p>
                                                <p className="text-lg font-bold text-slate-900">{vehicle.height} cm</p>
                                            </div>
                                        )}
                                        {vehicle.curb_weight && (
                                            <div className="bg-slate-50 p-4 rounded-2xl text-center">
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Curb Weight</p>
                                                <p className="text-lg font-bold text-slate-900">{vehicle.curb_weight} kg</p>
                                            </div>
                                        )}
                                        {vehicle.max_gross_weight && (
                                            <div className="bg-slate-50 p-4 rounded-2xl text-center">
                                                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Max Weight</p>
                                                <p className="text-lg font-bold text-slate-900">{vehicle.max_gross_weight} kg</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {(vehicle.winter_ready || vehicle.winter_tires || vehicle.studded_tires || vehicle.child_seat_space) && (
                                <div className="pt-4">
                                    <h4 className="text-xs font-bold uppercase tracking-widest text-amber-600 mb-4 flex items-center gap-2">
                            ...
                            {/* Book Button */}
                            <div className="pt-6">
                                <Button asChild className="w-full h-14 bg-slate-900 hover:bg-amber-500 text-white hover:text-white rounded-2xl font-bold transition-all shadow-lg hover:shadow-amber-500/20 text-lg">
